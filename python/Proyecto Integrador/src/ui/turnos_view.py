...
class TurnosView(ctk.CTkFrame):
    ...
    def _form_comun(self, dialogo, fila=None):
        ...
        nombres_proveedores = list(self.mapa_proveedores.values())
        nombres_servicios = list(self.mapa_servicios.values())

        dialogo.agregar_campo_label("Cliente (opcional — dejalo sin asignar para turno disponible)")
        combo_cliente = theme.combobox(
            dialogo.body, values=nombres_usuarios, width=340, state="readonly",
        )
        combo_cliente.set(nombres_usuarios[0])
        combo_cliente.pack(anchor="w")

        dialogo.agregar_campo_label("Proveedor")
        combo_proveedor = theme.combobox(
            dialogo.body, values=nombres_proveedores or ["Creá un proveedor primero"],
            width=340, state="readonly",
        )
        combo_proveedor.set(
            nombres_proveedores[0] if nombres_proveedores else "Creá un proveedor primero"
        )
        combo_proveedor.pack(anchor="w")

        dialogo.agregar_campo_label("Servicio")
        combo_servicio = theme.combobox(
            dialogo.body, values=nombres_servicios or ["Creá un servicio primero"],
            width=340, state="readonly",
        )
        combo_servicio.set(
            nombres_servicios[0] if nombres_servicios else "Creá un servicio primero"
        )
    ...
    def _abrir_dialogo_crear(self):
        ...
        def confirmar():
            ...
            if not (id_proveedor and id_servicio and fecha):
                if not self.mapa_proveedores or not self.mapa_servicios:
                    lbl_error.configure(
                        text="Primero creá al menos un proveedor y un servicio."
                    )
                else:
            ...
            try:
                ...
                lbl_error.configure(
                    text="Fecha inválida. Usá el formato AAAA-MM-DD HH:MM, "
                         "por ejemplo 2026-06-25 15:30."
                )
                return
            ...
            try:
                ...
                lbl_error.configure(text=f"No se pudo crear el turno: {e}")
                return
    ...
    def _abrir_dialogo_editar(self, fila):
        ...
        def confirmar():
            ...
            try:
                ...
                lbl_error.configure(text=f"No se pudo actualizar el turno: {e}")
                return
